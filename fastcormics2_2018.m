function [model, A_final] = fastcormics2_2018(model, data, rownames, dico, already_mapped_tag, consensus_proportion, epsilon, optional_settings)
...
global Cprev MapRepo;
Cprev = C;
MapRepo = sum(mapping, 2);
...
if ~isempty(optional_settings)&& isfield(optional_settings, 'func') 
    ...
    A = fastcore2_4_fastcormics(B, model, epsilon, C);
...
[A] = fastcc_4_fastcormics2(model,epsilon,0);
...
A2 = fastcore2_4_fastcormics(C, model_output, epsilon, t_keep);
